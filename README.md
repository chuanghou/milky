...
## 项目基本介绍
本项目定位为一个易用的ddd框架，笔者在朋友的介绍下接触了[Axon](https://www.axoniq.io/)，Axon本身为一个深度拥抱DDD，CQRS的的Java框架，不过研究过程中发现，推荐使用的JPA在国内也没啥人用，其深度封装导致上手有一定困难。还有一些其他原因，我决定自己写一个，开发历时近一年。本框架已经应用在两个项目中，线上运行均十分稳定，开发体验也相对不错。
本框架主要特点是通过领域能力封装，有效防止写面条代码。因为持久化是由框架调用，所以不可能出现乱七八糟调用的情况，所有领域能力封装到领域对象里面，而在领域对象里面是不能Autowired的
